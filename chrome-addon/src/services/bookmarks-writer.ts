...
function validateBookmarkWriteRequest(
  ...
  try {
    ...
    if (isDefined(bookmark) && isDefined(newBookmark)) {
      log(
...
async function deleteBookmark(params: ValidatedBookmarkDeleteRequest) {
  const [bookmark] = await chrome.bookmarks.get(params.id)
...
async function updateBookmark(params: ValidatedBookmarkUpdateRequest) {
   ...
   */
  const [{ parentId }] = await chrome.bookmarks.get(id)
  ...
  if (!targetFolder) {
     ...
     * create the bookmark in the target-folder
     */
    const updated = await chrome.bookmarks.move(id, {
      parentId: parentIdFolder,
    })
  ...
  await chrome.bookmarks.move(id, { parentId: parentIdFolder })
  return await deleteEmptyFolders(parentId)
}
...
async function deleteEmptyFolders(removedBookmarkParentId: string | undefined) {
  ...
  const [parent] = await chrome.bookmarks.get(removedBookmarkParentId)
  const children = await chrome.bookmarks.getChildren(removedBookmarkParentId)
  /* return if any child exists */
  if (
    /* 
    * no separator on chrome
    * @ see https://superuser.com/questions/633972/how-to-add-separator-to-google-chrome-bookmarks-bar
  ...
  await chrome.bookmarks.remove(removedBookmarkParentId)
...
async function findTargetFolder(folders: string[]): Promise<{
  targetId: string
  targetFolder: chrome.bookmarks.BookmarkTreeNode | undefined
  ...
  for (const [index, folder] of folders.entries()) {
    ...
    const matches = await chrome.bookmarks.search({ title: folder })
